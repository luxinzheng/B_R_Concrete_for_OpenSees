...
subroutine PSUMAT(nstatevs, nprops, props, stress, strain0, strain1, dstrain, statev, tangent)
    ...
    real(dp) :: eps_comp_max, eps_min_principal, eps_center, eps_radius
    ...
    !   matches all_adina.for: IF (PGRAV.EQ.1.D2) path
    !   Uses E = E0*STIFAC (reduced isotropic), hypoelastic update
    ! ==========================================================
    if (is_crushed) then
        m_E = E0 * m_STIFAC
        m_RK = m_E / (3.0d0*(1.0d0 - 2.0d0*m_VNU))
        ...
        EPSMM = -EVI / 3.0d0
        EPS11 = STRAIN(1) - EPSMM; EPS22 = STRAIN(2) - EPSMM
        EPS33 = STRAIN(4) - EPSMM; EPS12 = STRAIN(3)
        
        PEE = 3.0d0*m_RK*(EPSMM - EMM) + TMM
        S11 = 2.0d0*m_G*(EPS11 - E11) + T11
        S22 = 2.0d0*m_G*(EPS22 - E22) + T22
        S12 = m_G*(EPS12 - E12_old) + T12
        
        STRESS_NEW(1) = S11 + PEE
        STRESS_NEW(2) = S22 + PEE
        STRESS_NEW(3) = S12
        STRESS_NEW(4) = 0.0d0
    ...
    if (is_cracked) then
        ...
        ! Matches labels 5-12 with IKAS check
        ...
        ! --- B7a: Hybrid fixed-crack / isotropic-damage blend ---
        !   NUMCRK=0: aggressive drift correction (cracks closed)
        !   NUMCRK=1,2: blend cracked stress with isotropic total-strain
        !     stress to reduce the directional penalty of fixed cracks.
        !     This makes the model behave partly like a damage model,
        !     which better matches rotating-crack reference models.
        if (NUMCRK == 0) then
            alpha_b = 0.7d0
        else if (NUMCRK == 1 .or. NUMCRK == 5) then
            alpha_b = 0.25d0
        else if (NUMCRK == 2 .or. NUMCRK == 6) then
            alpha_b = 0.1d0
        else
            alpha_b = 0.0d0
        end if
        ...
        if (alpha_b > 0.0d0) then
            ...
            fac_b = E_sec_b / (1.0d0 - m_VNU**2)
            sig_t_b(1) = fac_b * (STRAIN(1) + m_VNU*STRAIN(2))
            sig_t_b(2) = fac_b * (STRAIN(2) + m_VNU*STRAIN(1))
            sig_t_b(3) = E_sec_b / (2.0d0*(1.0d0 + m_VNU)) * STRAIN(3)
            
            if (NUMCRK == 0) then
                STRESS_NEW(1) = (1.0d0-alpha_b)*STRESS_NEW(1) + alpha_b*sig_t_b(1)
                STRESS_NEW(2) = (1.0d0-alpha_b)*STRESS_NEW(2) + alpha_b*sig_t_b(2)
            end if
            STRESS_NEW(3) = (1.0d0-alpha_b)*STRESS_NEW(3) + alpha_b*sig_t_b(3)
        end if
        
        ! --- B8: Check for crushing ---
        call compute_principal_2d(STRESS_NEW, ANG)
        SIG3(1) = m_SIGP(1); SIG3(2) = m_SIGP(2); SIG3(3) = m_SIGP(4)
        call sort_principal(SIG3, P1, P2, P3)
        
        if (P2 < m_SIGCP .or. P3 < m_SIGCP) then
            NUMCRK = 3
            PGRAV  = PGRAV_CRUSH
            CRKSTR(1) = m_SIGCP
            CRKSTR(2) = m_EPSCP
        ...
        if (alpha_b > 0.0d0 .and. NUMCRK /= 3) then
            if (NUMCRK == 0) then
                fac_b = E_sec_b / (1.0d0 - m_VNU**2)
                tangent(1,1) = (1.0d0-alpha_b)*tangent(1,1) + alpha_b*fac_b
                tangent(1,2) = (1.0d0-alpha_b)*tangent(1,2) + alpha_b*fac_b*m_VNU
                tangent(2,1) = tangent(1,2)
                tangent(2,2) = (1.0d0-alpha_b)*tangent(2,2) + alpha_b*fac_b
                tangent(1,3) = (1.0d0-alpha_b)*tangent(1,3)
                tangent(2,3) = (1.0d0-alpha_b)*tangent(2,3)
                tangent(3,1) = tangent(1,3)
                tangent(3,2) = tangent(2,3)
            end if
    ...
    if (eps_min_principal < -1.0d-10) then
        ...
        if (P2 < m_SIGCP .or. P3 < m_SIGCP) NUMCRK = 3
    ...
    S11 = 2.0d0*m_G*(EPS11 - E11) + T11
    S22 = 2.0d0*m_G*(EPS22 - E22) + T22
    S33 = 2.0d0*m_G*(EPS33 - E33) + T33
    S12 = m_G*(EPS12 - E12_old) + T12
    
    STRESS_NEW(1) = S11 + PEE
    STRESS_NEW(2) = S22 + PEE
    STRESS_NEW(4) = S33 + PEE
    STRESS_NEW(3) = S12
    STRESS_NEW(4) = 0.0d0
